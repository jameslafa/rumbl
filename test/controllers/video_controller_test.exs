defmodule Rumbl.VideoControllerTest do
  ...
  test "lists all user's video on index", %{conn: conn, user: user} do
    ...
    other_video = insert_video(insert_user(username: "another guy"), title: "another video")
  ...
  test "does not create video and renders errors when invalid", %{conn: conn} do
    ...
    assert html_response(conn, 200) =~ "check the error"
    assert count_before == video_count(Video)
  end

  @tag login_as: "max"
  test "blocks users from updating someone else' video", %{conn: conn, user: owner} do
    video = insert_video(owner, @valid_attrs)

    non_owner = insert_user(username: "sneaky guy")
